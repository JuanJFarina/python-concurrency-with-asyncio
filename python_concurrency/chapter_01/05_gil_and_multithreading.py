...
# Python's Global Interpreter Lock (GIL) makes sure only one line of python
# code runs at a time inside a given process, making parallelism impossible for
# one given process. The reason for this is to prevent race conditions when
# python code make changes in the reference counter of the interpreter, meaning
# every time a reference to an object is created or removed. The need for all
# of this is because the internal CPython implementation is not thread-safe.

# While GIL prevents gaining benefits from multithreading in most CPU-bound
